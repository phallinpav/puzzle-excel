...
//        }
        ...
        int row = 0;
        int col = 0;
        String valCell = getCellStringValueAt(row, col);
        StringBuilder print = new StringBuilder();
        while(valCell.equals("X")) {
            print.append("X\t");
            col++;
            valCell = getCellStringValueAt(row, col);
        }
        maxCol = col;
        col = 0;
        row++;
...
//        done = true;
    ...
    private static void solution() {
        ...
        while (!queue.isEmpty()) {
            ...
            if (size == 0) {
                ...
                System.out.println("NEXT MOVES = " + size);
                System.out.println("-----------------------");
            }
        }
    ...
    private static void addColorToShortestPath() {
        if (done) {
            ...
            while (count > 0) {
                ...
                if (getCellValueAt(cell).equals((double) count)) {
    ...
    private static boolean checkCurrentMove(int row, int col) {
        String val = getCellStringValueAt(row, col);
        switch (val) {
            case "E":
                queue.clear();
